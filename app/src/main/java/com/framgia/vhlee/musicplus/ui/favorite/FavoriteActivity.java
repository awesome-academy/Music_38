...
public class FavoriteActivity extends AppCompatActivity
    ...
    @Override
    public void onGetTracksSuccess(List<Long> trackIds) {
        for (int i = 0; i < trackIds.size(); i++) {
            String api = StringUtil.initDetailApi(trackIds.get(i));
            mPresenter.loadFavorite(api);
        }
    }
    ...
    @Override
    public void onDeleteTracksSuccess(List<String> numberRows) {
    ...
    public void showFavorite(List<Track> tracks) {
        mTracks.add(tracks.get(0));
        mAdapter.notifyItemInserted(mTracks.size());
    }

    @Override
    public void showFavoriteFail(String message) {
